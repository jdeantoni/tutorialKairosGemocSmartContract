...
public class SimplelanguageModelStateHelper implements IK3ModelStateHelper{
	...
	public K3ModelState getK3ModelState(EObject model) {
		...
		Class<?> clazz =null;
		clazz = K3DslHelper.getTarget(fr.inria.kairos.tuto.simplelanguage.semantics.rewritingrules.SmartContractAspect.class);
		if (clazz.isInstance(model)) {
			ElementState elemState = theFactory.createElementState();
			elemState.setModelElement(model);
			res.getOwnedElementstates().add(elemState);
				AttributeNameToValue n2v0 = new AttributeNameToValue("currentState", SimplelanguageRTDAccessor.getcurrentState(model));
		...
		while (allContentIt.hasNext()) {
			...
			if (clazz.isInstance(elem)) {
				...
				res.getOwnedElementstates().add(elemState);
				AttributeNameToValue n2v0 = new AttributeNameToValue("currentState", SimplelanguageRTDAccessor.getcurrentState(elem));
				elemState.getSavedRTDs().add(n2v0);
			}
		}
		return res;
	...
	public void restoreModelState(K3ModelState state) {
		for(ElementState elemState : state.getOwnedElementstates()) {
			for(Object o : elemState.getSavedRTDs()) {
				...
				try {
					if (n2v.value != null) {
						Method m = SimplelanguageRTDAccessor.class.getMethod("set"+n2v.name, EObject.class, n2v.value.getClass());
					...
					for(Class<?> c : n2v.value.getClass().getInterfaces()) {
						...
						try {
							m = SimplelanguageRTDAccessor.class.getMethod("set"+n2v.name, EObject.class, n2v.value.getClass().getInterfaces()[0]);
							m.invoke(null, elemState.getModelElement(), n2v.value);
